...
<div class="col-md-12">
    ...
    <?php
    if(!empty($model->ir_tchnID)){
        echo '<div class="col-md-6 cus-border"><h4>เจ้าหน้าที่ซ่อม := <u>'.$model->irTchn->getFullname().'</u></h4></div>';
    }else{
        echo '<div class="col-md-6 cus-border"><h4>เจ้าหน้าที่ซ่อม := <u></u></h4></div>';
    }
    //echo '<div class="col-md-6 cus-border"><h4>เจ้าหน้าที่ซ่อม := <u>'.(!empty($model->ir_tchnID)) ? $model->irTchn->getFullname() : 'fsdfs'.'</u></h4></div>';
...
<?php if(!isset($model->ir_shopsuggestdate) or $model->status > 2 /**/){
    if(Yii::$app->controller->action->id == 'toexec') {
        ...
        echo '</div>';
    }else if($model->status > 2 and isset($model->ir_shopsuggestdate)){
...
<?php } ?>
<?php if(!isset($model->ir_tchnsuggest) or $model->status > 2 /**/){

    if(Yii::$app->controller->action->id == 'shoptchn') {
        echo '<div class="col-md-12">';
        echo '<h4><span class="glyphicon glyphicon-comment"></span> ความเห็นช่างหลังร้านตรวจ/เสนอ</h4>';
        echo $form->field($model, 'ir_tchnsuggest', [
        ...
        echo '</div>';
    }else if(isset($model->ir_tchnsuggest)){
        echo '<div class="col-md-12">';
        echo '<div class="col-md-12 cus-border">';
        echo '<h4>ความเห็นช่างหลังร้านตรวจ/เสนอ :=</h4> '.$model->ir_tchnsuggest;
        echo '</div></div>';
    } ?>
<?php } ?>
